...
def parse_momo_statement(statement_text: str, tool_context: ToolContext) -> dict:
    ...
    alerts = re.findall(r'(.*?(?:Transaction ID:|txn:|ID:)\s*\d+)', normalized_text, flags=re.IGNORECASE)
    ...
    for alert in alerts:
        ...
        txn_id_match = re.search(r'(?:Transaction ID:|txn:|ID:)\s*(\d+)', alert, re.IGNORECASE)
        if txn_id_match:
        ...
        # 2. Extract Amount
        amount_match = re.search(r'(?:GHS|GHC|Ghs)\s*(-?[\d,]+\.\d{2})', alert, re.IGNORECASE)
        if amount_match:
        ...
        # Check for Recieved/Cash In
        if re.search(r'(?:Received|Cash In|deposited|From|transfer from)', alert, re.IGNORECASE):
            txn["type"] = "deposit"
            # Try to extract sender
            sender_match = re.search(r'(?:from|From)\s+([A-Za-z0-9\s\-]+?)\s+(?:on|Transaction|Current|\.)', alert)
            txn["counterparty"] = sender_match.group(1).strip() if sender_match else "Unknown Sender"
        # Check for Paid/Cash Out
        elif re.search(r'(?:paid|sent|Cash Out|transfer to|payment to)', alert, re.IGNORECASE):
            txn["type"] = "withdrawal"
            # Try to extract recipient
            recipient_match = re.search(r'(?:to|To)\s+([A-Za-z0-9\s\-]+?)\s+(?:on|Transaction|Current|\.)', alert)
            txn["counterparty"] = recipient_match.group(1).strip() if recipient_match else "Unknown Recipient"
        else:
            txn["type"] = "other"
    ...
    for t in transactions:
        t["counterparty"] = re.sub(r'\b0\d{9}\b|\b233\d{9}\b', "[ANONYMIZED_PHONE]", t["counterparty"])

    # Store in session state for downstream skills
    tool_context.state["parsed_transactions"] = transactions
