...
def extract_momo_from_image(image_path: str, tool_context: ToolContext) -> dict:
    ...
    if not re.search(r'\.(png|jpe?g)$', image_path, re.IGNORECASE):
        return {"status": "error", "message": "Unsupported file format. Only PNG, JPG, and JPEG are supported."}
    ...
    try:
        ...
        Example SMS formats:
        - "Received GHS 150.00 from Kwame Mensah on 2026-06-29 14:20:00. Transaction ID: 987654321."
        - "You have paid GHS 50.00 to ECG Prepaid. Transaction ID: 11223344."
        - "Cash Out GHS 200.00 to agent Mom's Shop. Transaction ID: 88990011."
        ...
        if not extracted_text or not extracted_text.strip():
            return {"status": "error", "message": "No transaction text could be extracted from the screenshot."}
        ...
        return parse_momo_statement(statement_text=extracted_text, tool_context=tool_context)

    except Exception as e:
        return {"status": "error", "message": f"Failed to process screenshot image: {str(e)}"}
