...
DATABASE_FILE = os.path.join(
    os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "momo.db"
)
...
def save_transaction(
    ...
    # Database-level Defense in Depth PII scrubbing
    import re

    # Mask any phone numbers that slipped through in counterparty
    scrubbed_counterparty = re.sub(
        r"\b0\d{9}\b|\b233\d{9}\b|\b\d{9,12}\b", "[REDACTED_PHONE]", counterparty
    )

    # Scrub raw payload if present
    scrubbed_payload = raw_payload
    if raw_payload:
        scrubbed_payload = re.sub(
            r"\b0\d{9}\b|\b233\d{9}\b|\b\d{9,12}\b", "[REDACTED_PHONE]", raw_payload
        )
    ...
    try:
        ...
        new_data = {
            ...
            "amount": amount,
            "direction": direction,
            "counterparty": scrubbed_counterparty,
            "category": category,
            "confidence": confidence,
        }
        log_audit(
            conn, merchant_id, "INSERT", "transactions", txn_id, new_data=new_data
        )
...
def get_merchant_transactions(merchant_id: str) -> list:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT * FROM transactions WHERE merchant_id = ? ORDER BY timestamp DESC",
        (merchant_id,),
    )
...
def save_merchant(
    merchant_id: str, business_name: str, owner_name: str, phone: str
) -> None:
...
def get_financial_summary(merchant_id: str) -> dict[str, Any] | None:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT * FROM financial_summaries WHERE merchant_id = ? ORDER BY calculated_at DESC LIMIT 1",
        (merchant_id,),
    )
    row = cursor.fetchone()
    conn.close()
...
def get_credit_profile(merchant_id: str) -> dict[str, Any] | None:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT * FROM credit_profiles WHERE merchant_id = ? ORDER BY created_at DESC LIMIT 1",
        (merchant_id,),
    )
    row = cursor.fetchone()
    conn.close()
...
def update_transaction_category(
    transaction_id: str, merchant_id: str, category: str
) -> bool:
