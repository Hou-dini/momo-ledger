...
async def generate_ledger_report(tool_context: ToolContext) -> dict:
    ...
    if not categorized or not metrics:
        return {
            "status": "error",
            "message": "Missing transaction or credit analysis metrics.",
        }
    ...
    report.append(
        f"* **Total Business Inflows (Revenues)**: GHS {metrics['total_inflow']:,.2f}"
    )
    report.append(
        f"* **Total Business Outflows (Expenses)**: GHS {metrics['total_outflow']:,.2f}"
    )
    ...
    part = types.Part(
        inline_data=types.Blob(mime_type="text/markdown", data=report_bytes)
    )
